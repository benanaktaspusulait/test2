@EventSnsCVeh2Ite
Feature: Test sns Command Adaptor - Event - Association Start - SNS-ASSOC-CVeh2Ite
  ...
  Scenario Outline: EventRecord - AssociationStart - SNS-ASSOC-CVeh2Ite
    ...
    Then 1 Event Commands for SNS-ASSOC-CVeh2Ite will be emitted
    And one Event record for "SNS-ASSOC-CVeh2Ite" with following attributes
      | metadata.identityRecord.poleId.v2.id          | SNSENS:E={[SNSENS:P=null[{Nationalityvehicle001}],O={Nationalityvehicle001}][SNSENS:S={movementReferenceNumberIN1234}][OTOSASSO]} |
      ...
      | metadata.sourceRecord.id                      | {[SNSENS:P=null[{Nationalityvehicle001}],O={Nationalityvehicle001}][SNSENS:S={movementReferenceNumberIN1234}][OTOSASSO]}          |
  ...
  Scenario Outline: STOP: SNS-ASSOC-CVeh2Ite not created if identity or mode is not 2,3,10,11
    When the background template of Sns data is changed to
      | field                                                              | value      |
      | body.goods.goodsItems.0.identityOfMeansOfCrossingBorder.0.identity | <identity> |
      | body.itinerary.modeOfTransportAtBorder                             | <mode>     |
    And StreamIngestRecord source data is presented with attributes as per the template to the fdp-sns-input_0 input topic
    Then 0 Event Commands for SNS-ASSOC-CVeh2Ite will be emitted
    Examples:
      | identity              | mode |
      | [blank]               | 2    |
      | null                  | 2    |
