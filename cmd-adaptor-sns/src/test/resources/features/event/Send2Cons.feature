@EventSnsSend2Cons
Feature: Test sns Command Adaptor - Event - Association Start - SNS-ASSOC-Send2Cons
  ...
  Background:
    Given template StreamIngestRecord source data with the following attributes
      ...
      | body.metadata.senderEORI                                              | senderEORI                                                       |
  ...
  Scenario: EventRecord - AssociationStart - SNS-ASSOC-Send2Cons
    When StreamIngestRecord source data is presented with attributes as per the template to the fdp-sns-input_0 input topic
    Then 1 Event Commands for SNS-ASSOC-Send2Cons will be emitted
    And one Event record for "SNS-ASSOC-Send2Cons" with following attributes
      | metadata.identityRecord.poleId.v2.id          | SNSENS:E={[SNSENS:P={senderEORI}[SNS-SENDER]][movementReferenceNumber123][PTOS]} |
      ...
      | metadata.sourceRecord.id                      | {[SNSENS:P={senderEORI}[SNS-SENDER]][movementReferenceNumber123][PTOS]}          |
      ...
      | subject.poleId.v2.id                          | SNSENS:P={senderEORI}[SNS-SENDER]                                                |
      | subject.type                                  | S                                                                                |
  ...
  Scenario Outline: EventRecord - AssociationStart - SNS-ASSOC-Send2Cons
    ...
    Then 1 Event Commands for SNS-ASSOC-Send2Cons will be emitted
    And one Event record for "SNS-ASSOC-Send2Cons" with following attributes
      | metadata.identityRecord.poleId.v2.id          | SNSENS:E={[SNSENS:P={senderEORI}[SNS-SENDER]][movementReferenceNumber<refNum>][PTOS]} |
      | metadata.identityRecord.type                  | E                                                                                     |
      | metadata.sourceRecord.name                    | SNSENS                                                                                |
      | metadata.sourceRecord.shortName               | SNS                                                                                   |
      | metadata.sourceRecord.location                | submissionIdmetadata.messageIdentification                                            |
      | metadata.sourceRecord.id                      | {[SNSENS:P={senderEORI}[SNS-SENDER]][movementReferenceNumber<refNum>][PTOS]}          |
      | metadata.sourceRecord.audit.createdBy         | 0123456789                                                                            |
      | metadata.sourceRecord.audit.createdTimestamp  | 2022-02-22T22:22:22.222Z                                                              |
      | metadata.mappingRecord.name                   | SNS-ASSOC-Send2Cons                                                                   |
      | metadata.mappingRecord.version                | mappingVersion                                                                        |
      ...
      | subject.type                                  | S                                                                                     |
      | associationStart.type                         | PTOS                                                                                  |
      | associationStart.target.poleId.v2.id          | SNSENS:S={movementReferenceNumber<refNum>}                                            |
      | associationStart.target.type                  | S                                                                                     |
      | associationStart.timestamp                    | 2022-02-22T22:22:22.222Z                                                              |
      | attributes.attrs.header.ingestDateTime        | 2022-02-22T22:22:22.222Z                                                              |

    Examples:
      | mode | item   | refNum | itRefNum | goodsRefNum |
      | 10   | IN1234 | 123    | 789      | 123         |
      | 10   | IN1234 | 789    | 789      | null        |
      | 10   | IN1234 | 12345  | null     | null        |
      | 11   | IN1234 | 789    | 789      | 123         |
      | 11   | IN1234 | 123    | null     | 123         |
      | 11   | IN1234 | 12345  | null     | null        |

  #DEV NOTE - below STOP disabled because body.metadata.senderEORI in current AVRO cannot be set to null
#  Scenario: STOP: SNS-ASSOC-Send2Cons not created if name and eori are null
#    When the background template of Sns data is changed to
#      | field                       | value |
#      | body.metadata.senderEORI    | null  |
#
#    And StreamIngestRecord source data is presented with attributes as per the template to the fdp-sns-input_0 input topic
#    Then 0 Event Commands for SNS-ASSOC-Send2Cons will be emitted
