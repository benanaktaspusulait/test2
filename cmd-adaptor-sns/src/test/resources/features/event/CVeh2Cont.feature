@EventSnsVeh2Cons
Feature: Test sns Command Adaptor - Event - Association Start - SNS-ASSOC-CVeh2Cont
  ...
  Scenario Outline: EventRecord - AssociationStart - SNS-ASSOC-CVeh2Cont
    ...
    Then 1 Event Commands for SNS-ASSOC-CVeh2Cont will be emitted
    And one Event record for "SNS-ASSOC-CVeh2Cont" with following attributes
      | metadata.identityRecord.poleId.v2.id          | SNSENS:E={[SNSENS:P=null[{Nationalityvehicle001}],O={Nationalityvehicle001}][SNSENS:P=null[{movementReferenceNumbercont0}],O={movementReferenceNumbercont0}][OTOOLINK]} |
      ...
      | metadata.sourceRecord.id                      | {[SNSENS:P=null[{Nationalityvehicle001}],O={Nationalityvehicle001}][SNSENS:P=null[{movementReferenceNumbercont0}],O={movementReferenceNumbercont0}][OTOOLINK]}          |
      ...
      | associationStart.target.poleId.v2.id          | SNSENS:P=null[{movementReferenceNumbercont0}],O={movementReferenceNumbercont0}                                                                                         |
  ...
  Scenario Outline: STOP: SNS-ASSOC-CVeh2Cont not created if identity/container is blank or null, or mode is not 2,3,10,11
    When the background template of Sns data is changed to
      | field                                                              | value       |
      | body.goods.goodsItems.0.identityOfMeansOfCrossingBorder.0.identity | <identity>  |
      | body.itinerary.modeOfTransportAtBorder                             | <mode>      |
      | body.goods.goodsItems.0.containers.0.containerNumber               | <container> |
    And StreamIngestRecord source data is presented with attributes as per the template to the fdp-sns-input_0 input topic
    Then 0 Event Commands for SNS-ASSOC-CVeh2Cont will be emitted
    Examples:
      | identity              | mode | container |
      | [blank]               | 2    | cont0     |
      | null                  | 2    | cont0     |
      | vehicle001#trailer001 | 2    | [blank]   |
      | vehicle001#trailer001 | 2    | null      |
      | vehicle001#trailer001 | 4    | cont0     |
      | vehicle001#trailer001 | 1    | cont0     |


  Scenario: STOP: SNS-ASSOC-CVeh2Cont not created if modeOfTransportAtBorder is equal to "1"
    When the background template of Sns data is changed to
      | field                                  | value |
