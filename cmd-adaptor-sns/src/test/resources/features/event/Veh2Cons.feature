@EventSnsVeh2Cons
Feature: Test sns Command Adaptor - Event - Association Start - SNS-ASSOC-Veh2Cons
  ...
  Scenario: EventRecord - AssociationStart - SNS-ASSOC-Veh2Cons
    When StreamIngestRecord source data is presented with attributes as per the template to the fdp-sns-input_0 input topic
    Then 1 Event Commands for SNS-ASSOC-Veh2Cons will be emitted
    And one Event record for "SNS-ASSOC-Veh2Cons" with following attributes
      | metadata.identityRecord.poleId.v2.id          | SNSENS:E={[SNSENS:P=null[{Nationalityvehicle001}],O={Nationalityvehicle001}][SNSENS:S={movementReferenceNumber123}][OTOSASSO]} |
      ...
      | metadata.sourceRecord.id                      | {[SNSENS:P=null[{Nationalityvehicle001}],O={Nationalityvehicle001}][SNSENS:S={movementReferenceNumber123}][OTOSASSO]}          |
...
#  Scenario: STOP: SNS-ASSOC-Veh2Cons not created if identity is shorter than 10 characters
...
#    Then 0 Event Commands for SNS-ASSOC-Veh2Cons will be emitted

  Scenario: STOP: SNS-ASSOC-Veh2Cons not created if modeOfTransportAtBorder is equal to "1"
    When the background template of Sns data is changed to
      | field                                  | value |
