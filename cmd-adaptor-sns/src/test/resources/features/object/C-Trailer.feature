@objectCTrailer
Feature: SNS Command Adaptor - Object - SNS-C-TRAILER
  ...
  Background:
    Given template StreamIngestRecord source data with the following attributes
      ...
      | body.itinerary.identityOfMeansOfCrossingBorder.identity               | vehicle002#trailer002                                         |
  ...
  Scenario Outline: STOP: SNS-C-TRAILER not created if modeOfTransportAtBorder not in 2,3,10,11
    ...
    Then 0 Object Commands for SNS-C-TRAILER will be emitted
    Examples:
      | mode |
      | 4    |

  Scenario Outline: STOP: SNS-C-TRAILER not created if identity length is 10 or whitespace or Trailer part is null
    When the background template of Sns data is changed to
      | field                                                              | value   |
      | body.goods.goodsItems.0.identityOfMeansOfCrossingBorder.0.identity | <value> |
    And StreamIngestRecord source data is presented with attributes as per the template to the fdp-sns-input_0 input topic
    Then 0 Object Commands for SNS-C-TRAILER will be emitted
    Examples:
      | value       |
