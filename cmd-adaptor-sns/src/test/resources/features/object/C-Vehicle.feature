@objectCVehicle
Feature: SNS Command Adaptor - Object - SNS-C-VEHICLE
  ...
  Scenario: STOP: SNS-C-VEHICLE not created if modeOfTransportAtBorder=1
    ...
    Then 0 Object Commands for SNS-C-VEHICLE will be emitted

  Scenario: STOP: SNS-C-VEHICLE not created if vehicleSplit returns null due to invalid value in goodsItems.0.identityOfMeansOfCrossingBorder.0.identity
    When the background template of Sns data is changed to
      | field                                                              | value           |
      | body.goods.goodsItems.0.identityOfMeansOfCrossingBorder.0.identity | VehicleTrailer  |
    And StreamIngestRecord source data is presented with attributes as per the template to the fdp-sns-input_0 input topic
    Then 0 Object Commands for SNS-C-VEHICLE will be emitted
  ...
  # FOR OBJVEHC(itinerary.identityOfMeansOfCrossingBorder.identity), if length > 9, split field on " " or "#" take left split else use full field
    When the background template of Sns data is changed to
      | field                                                              | value            |
      | body.goods.goodsItems.0.identityOfMeansOfCrossingBorder.0.identity | <actualIdentity> |
    And StreamIngestRecord source data is presented with attributes as per the template to the fdp-sns-input_0 input topic
    Then <expectNumberOfVehicle> Object Commands for SNS-C-VEHICLE will be emitted
    And one Object record for "SNS-C-VEHICLE" with following attributes
