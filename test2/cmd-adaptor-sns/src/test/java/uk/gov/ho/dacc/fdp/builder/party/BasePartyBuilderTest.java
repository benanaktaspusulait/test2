...
public class BasePartyBuilderTest {
    ...
    public void buildCommand_shouldBuildKeyAndCommandWithExpectedMetadata() throws Exception {
        ...
        PoleV2IdRecord expectedPoleV2Id = PoleV2IdRecord.newBuilder().setId("SNSENS:P=123").build();
        ...
        TestBasePartyBuilder builder = new TestBasePartyBuilder(input -> new AvroMutator<>(partyRecord));
        ...
        KeyValue<PoleV2IdRecord, CmdPartyPoleRecord> result = builder.buildCommand(new Object());

        assertEquals(expectedPoleV2Id, result.key);
        assertEquals(CmdType.MAP_PARTY_CMD, result.value.getCmdType());
        ...
        assertEquals(expectedPoleV2Id, result.value.getPartyRecord().getMetadata().getIdentityRecord().getPoleId().getV2());

        ComplianceRecord compliance = result.value.getPartyRecord().getMetadata().getComplianceRecord();
    ...
    private static class TestBasePartyBuilder extends BasePartyBuilder {
        private final ITransform<Object, AvroMutator<PartyRecord>> transformer;

        private TestBasePartyBuilder(ITransform<Object, AvroMutator<PartyRecord>> transformer) {
            this.transformer = transformer;
        }

        @Override
        protected String getCommandBuilderNameForMetrics() {
            return "test_party_builder";
        }

        @Override
        @SuppressWarnings("unchecked")
        protected <S> ITransform<S, AvroMutator<PartyRecord>> getTransformer() {
            return (ITransform<S, AvroMutator<PartyRecord>>) transformer;
        }
    }
}
