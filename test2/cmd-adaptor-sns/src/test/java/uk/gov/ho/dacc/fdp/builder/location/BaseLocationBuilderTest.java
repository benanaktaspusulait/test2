...
public class BaseLocationBuilderTest {
    ...
    public void buildCommand_shouldBuildKeyAndCommandWithExpectedMetadata() throws Exception {
        ...
        PoleV2IdRecord expectedPoleV2Id = PoleV2IdRecord.newBuilder().setId("SNSENS:P=123,L=123").build();
        ...
        TestBaseLocationBuilder builder = new TestBaseLocationBuilder(input -> new AvroMutator<>(locationRecord));
        ...
        KeyValue<PoleV2IdRecord, CmdLocationPoleRecord> result = builder.buildCommand(new Object());

        assertEquals(expectedPoleV2Id, result.key);
        assertEquals(CmdType.MAP_LOCATION_CMD, result.value.getCmdType());
        ...
        assertEquals(expectedPoleV2Id, result.value.getLocationRecord().getMetadata().getIdentityRecord().getPoleId().getV2());

        ComplianceRecord compliance = result.value.getLocationRecord().getMetadata().getComplianceRecord();
    ...
    private static class TestBaseLocationBuilder extends BaseLocationBuilder {
        private final ITransform<Object, AvroMutator<LocationRecord>> transformer;

        private TestBaseLocationBuilder(ITransform<Object, AvroMutator<LocationRecord>> transformer) {
            this.transformer = transformer;
        }

        @Override
        protected String getCommandBuilderNameForMetrics() {
            return "test_location_builder";
        }

        @Override
        @SuppressWarnings("unchecked")
        protected <S> ITransform<S, AvroMutator<LocationRecord>> getTransformer() {
            return (ITransform<S, AvroMutator<LocationRecord>>) transformer;
        }
    }
}
