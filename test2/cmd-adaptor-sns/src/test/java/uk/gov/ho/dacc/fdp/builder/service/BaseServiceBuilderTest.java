...
public class BaseServiceBuilderTest {
    ...
    public void buildCommand_shouldBuildKeyAndCommandWithExpectedMetadata() throws Exception {
        ...
        PoleV2IdRecord expectedPoleV2Id = PoleV2IdRecord.newBuilder().setId("SNSENS:S=123").build();
        ...
        TestBaseServiceBuilder builder = new TestBaseServiceBuilder(input -> new AvroMutator<>(serviceRecord));
        ...
        KeyValue<PoleV2IdRecord, CmdServicePoleRecord> result = builder.buildCommand(new Object());

        assertEquals(expectedPoleV2Id, result.key);
        assertEquals(CmdType.MAP_SERVICE_CMD, result.value.getCmdType());
        ...
        assertEquals(expectedPoleV2Id, result.value.getServiceRecord().getMetadata().getIdentityRecord().getPoleId().getV2());

        ComplianceRecord compliance = result.value.getServiceRecord().getMetadata().getComplianceRecord();
    ...
    private static class TestBaseServiceBuilder extends BaseServiceBuilder {
        private final ITransform<Object, AvroMutator<ServiceRecord>> transformer;

        private TestBaseServiceBuilder(ITransform<Object, AvroMutator<ServiceRecord>> transformer) {
            this.transformer = transformer;
        }

        @Override
        protected String getCommandBuilderNameForMetrics() {
            return "test_service_builder";
        }

        @Override
        @SuppressWarnings("unchecked")
        protected <S> ITransform<S, AvroMutator<ServiceRecord>> getTransformer() {
            return (ITransform<S, AvroMutator<ServiceRecord>>) transformer;
        }
    }
}
