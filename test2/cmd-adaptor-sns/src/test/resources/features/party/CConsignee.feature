@OrganisationSnsCConsignee
Feature: Test sns Command Adaptor - Party - Organisation - SNS-C-CONSIGNEE
  ...
  Background:
    Given template StreamIngestRecord source data with the following attributes
      ...
      | body.goods.goodsItems.0.consignee.address.postalCode                  | LD123ON                                                          |
  ...
  Scenario: PartyRecord - Organisation - SNS-C-CONSIGNEE
    When StreamIngestRecord source data is presented with attributes as per the template to the fdp-sns-input_0 input topic
    Then 1 Party Commands for SNS-C-CONSIGNEE will be emitted
    And one Party record for "SNS-C-CONSIGNEE" with following attributes
      | metadata.identityRecord.poleId.v2.id                 | SNSENS:P={goodsConsigneeName15LondonLD123ON100}[SNS-C-CONSIGNEE] |
      ...
      | metadata.sourceRecord.id                             | {goodsConsigneeName15LondonLD123ON100}                           |
  ...
  Scenario: SNS-C-CONSIGNEE is created if eori is not null
    When the background template of Sns data is changed to
      | field                                  | value |
      | body.goods.goodsItems.0.consignee.eori | BA444 |
    And StreamIngestRecord source data is presented with attributes as per the template to the fdp-sns-input_0 input topic
    Then 1 Party Commands for SNS-C-CONSIGNEE will be emitted
    And one Party record for "SNS-C-CONSIGNEE" with following attributes
      | metadata.identityRecord.poleId.v2.id                 | SNSENS:P={BA444}[SNS-C-CONSIGNEE]          |
      | metadata.identityRecord.type                         | P                                          |
      | metadata.sourceRecord.name                           | SNSENS                                     |
      | metadata.sourceRecord.shortName                      | SNS                                        |
      | metadata.sourceRecord.location                       | submissionIdmetadata.messageIdentification |
      | metadata.sourceRecord.id                             | {BA444}                                    |
      | metadata.sourceRecord.audit.createdBy                | 0123456789                                 |
      | metadata.sourceRecord.audit.createdTimestamp         | 2022-02-22T22:22:22.222Z                   |
      | metadata.mappingRecord.name                          | SNS-C-CONSIGNEE                            |
      | metadata.mappingRecord.version                       | mappingVersion                             |
      ...
      | organisation.name                                    | goodsConsigneeName                         |
      | organisation.registrationNumber                      | null                                       |
      | organisation.vatNumber                               | BA444                                      |
      | organisation.industrySector                          | null                                       |
      | organisation.numberOfEmployees                       | null                                       |
      | attributes.attrs.header.ingestDateTime               | 2022-02-22T22:22:22.222Z                   |
      | attributes.attrs.goods.goodsItems.consignee.language | goods.goodsItems.0.consignee.language      |
      | attributes.attrs.goods.goodsItems.consignee.eori     | BA444                                      |
      | attributes.attrs.eoriExtractionType                  | BA444                                      |
      | attributes.attrs.eoriPrincipalEconomicActivity       | BA444                                      |
      | attributes.attrs.eoriVatRegistrationNumber           | BA444                                      |
      | attributes.attrs.eoriSendingDateTime                 | BA444                                      |
      | attributes.attrs.eoriDateOfRegistration              | BA444                                      |
      | attributes.attrs.eoriTraderName                      | BA444                                      |
