@OrganisationSnsMShipper
Feature: Test sns Command Adaptor - Party - Organisation - SNS-M-SHIPPER
  ...
  Scenario: PartyRecord - Organisation - SNS-M-SHIPPER
    When StreamIngestRecord source data is presented with attributes as per the template to the fdp-sns-input_0 input topic
    Then 1 Party Commands for SNS-M-SHIPPER will be emitted
    And one Party record for "SNS-M-SHIPPER" with following attributes
      | metadata.identityRecord.poleId.v2.id          | SNSENS:P={partiesConsignorName90CardiffCA34 7FF100}[SNS-M-SHIPPER] |
      ...
      | metadata.sourceRecord.id                      | {partiesConsignorName90CardiffCA34 7FF100}                         |
      ...
      | organisation.name                             | partiesConsignorName                                               |
  ...
  Scenario: SNS-M-SHIPPER is created if eori is not null
    ...
    And one Party record for "SNS-M-SHIPPER" with following attributes
      ...
      | attributes.attrs.header.ingestDateTime         | 2022-02-22T22:22:22.222Z                   |
      | attributes.attrs.parties.consignor.language    | parties.consignor.language                 |
      | attributes.attrs.parties.consignor.eori        | EORI56                                     |
  ...
  Scenario: STOP: SNS-M-SHIPPER not created if name and eori are null
    When the background template of Sns data is changed to
      | field                       | value |
      | body.parties.consignor.name | null  |
      | body.parties.consignor.eori | null  |
    And StreamIngestRecord source data is presented with attributes as per the template to the fdp-sns-input_0 input topic
    Then 0 Party Commands for SNS-M-SHIPPER will be emitted
