@EventSnsVess2Cons
Feature: Test sns Command Adaptor - Event - Association Start - SNS-ASSOC-Vess2Cons
  ...
  Background:
    Given template StreamIngestRecord source data with the following attributes
      ...
      | body.goods.goodsItems.0.identityOfMeansOfCrossingBorder.0.language    | #2345678                                                         |
      ...
      | body.itinerary.identityOfMeansOfCrossingBorder.identity               | #1234567                                                         |
  ...
  Scenario: EventRecord - AssociationStart - SNS-ASSOC-Vess2Cons
    When StreamIngestRecord source data is presented with attributes as per the template to the fdp-sns-input_0 input topic
    Then 1 Event Commands for SNS-ASSOC-Vess2Cons will be emitted
    And one Event record for "SNS-ASSOC-Vess2Cons" with following attributes
      | metadata.identityRecord.poleId.v2.id          | SNSENS:E={[SNSENS:P=null[{1234567Nationality}],O={1234567Nationality}][SNSENS:S={movementReferenceNumber123}][OTOSASSO]} |
      ...
      | metadata.sourceRecord.id                      | {[SNSENS:P=null[{1234567Nationality}],O={1234567Nationality}][SNSENS:S={movementReferenceNumber123}][OTOSASSO]}          |
      | metadata.sourceRecord.audit.createdBy         | 0123456789                                                                                         |
      | metadata.sourceRecord.audit.createdTimestamp  | 2022-02-22T22:22:22.222Z                                                                           |
      | metadata.mappingRecord.name                   | SNS-ASSOC-Vess2Cons                                                                                |
      ...
      | subject.poleId.v2.id                          | SNSENS:P=null[{1234567Nationality}],O={1234567Nationality}                                                               |
      | subject.type                                  | O                                                                                                  |
  ...
  Scenario: STOP: SNS-ASSOC-Vess2Cons not created if modeOfTransportAtBorder is not equal to "1"
    When the background template of Sns data is changed to
      | field                                  | value |
      | body.itinerary.modeOfTransportAtBorder | 3     |
    And StreamIngestRecord source data is presented with attributes as per the template to the fdp-sns-input_0 input topic
    Then 0 Event Commands for SNS-ASSOC-Vess2Cons will be emitted

  Scenario: STOP: SNS-ASSOC-Vess2Cons  not created if length identityOfMeansOfCrossingBorder.identity >= 10
    When the background template of Sns data is changed to
      | field                                                      | value       |
      | body.itinerary.identityOfMeansOfCrossingBorder.identity    | identity901 |
      | body.itinerary.identityOfMeansOfCrossingBorder.nationality | Nationality |
    And StreamIngestRecord source data is presented with attributes as per the template to the fdp-sns-input_0 input topic
    Then 0 Event Commands for SNS-ASSOC-Vess2Cons will be emitted
