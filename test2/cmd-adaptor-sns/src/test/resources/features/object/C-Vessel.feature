@objectCVessel
Feature: SNS Command Adaptor - Object - SNS-C-VESSEL
  ...
  Background:
    Given template StreamIngestRecord source data with the following attributes
      ...
      | body.itinerary.identityOfMeansOfCrossingBorder.identity               | test                                                          |
  ...
  Scenario Outline: STOP: SNS-M-VESSEL not created if modeOfTransportAtBorder is not 1 (maritime) or 11 (Unacc RoRo)
    ...
    Then 0 Object Commands for SNS-C-VESSEL will be emitted
    Examples:
      | mode |
      | 2    |
      | 56   |

  Scenario Outline: STOP: SNS-M-VESSEL  not created if length identityOfMeansOfCrossingBorder.identity >= 10
    When the background template of Sns data is changed to
      | field                                                                  | value       |
      | body.goods.goodsItems.0.identityOfMeansOfCrossingBorder.0.identity     | <identity>  |
      | body.igoods.goodsItems.0.identityOfMeansOfCrossingBorder.0.nationality | Nationality |
      | body.itinerary.modeOfTransportAtBorder                                 | 1           |
    And StreamIngestRecord source data is presented with attributes as per the template to the fdp-sns-input_0 input topic
    Then 0 Object Commands for SNS-C-VESSEL will be emitted
    Examples:
